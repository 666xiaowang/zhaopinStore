...
<body>
  	<div id="previewWrapper">
        <div class="preview_header">
            <h1 title="jason的简历">${sessionScope.user.name}发布的职位</h1>
                    </div><!--end .preview_header-->

        <div class="preview_content">
            <div class="profile_box" id="basicInfo">
                <h2>${sessionScope.user.name}发布的职位</h2>
                <div class="basicShow">
                <c:forEach	items="${sessionScope.positionList}" var="position" varStatus="status">
                	<br />
                	<center><h2>-----第${status.count}个职位-----</h2></center>
                	<table width="100%" border="1" cellSpacing="0">
                		<tr>
                			<td>职位类型:${position.positionType }</td>
                			<td>职位名称:${position.positionName }</td>
                		</tr>
                		<tr>
                			<td>所属部门:${position.department }</td>
                			<td>工作性质:${position.jobNature }</td>
                		</tr>
                		<tr>
                			<td>最低薪水:${position.salaryMin }</td>
                			<td>最高薪水:${position.salaryMax }</td>
                		</tr>
                		<tr>
                			<td>工作城市:${position.workAddress }</td>
                			<td>工作经历:${position.experience }</td>
                		</tr>
                		<tr>
                			<td>学历要求:${position.education }</td>
                			<td>职位诱惑:${position.positionAdvantage }</td>
                		</tr>
                		<tr>
                			<td>职位描述:${position.positionDetail }</td>
                			<td>工作地址:${position.positionAddress }</td>
                		</tr>
                		<tr>
                			<td colspan="2">接收简历邮箱:${position.forwardEmail  }</td>
                		</tr>
                	</table>
                	<br />
                </c:forEach>
